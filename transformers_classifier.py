...
class DocumentClassifier:
    def __init__(self, net, tokenizer, vocab, num_labels, max_length=512, batch_size=32, num_epochs=100, random_seed=None):
    ...
    def fit(self, train_df, val_df, optimizer, early_stopping_rounds=10):
        logger.info('[Start]Create DataSets from DataFrames')
        train_ds = DataFrameDataset(
            train_df, fields={'Text': self.TEXT, 'Label': self.LABEL})
        val_ds = DataFrameDataset(
            val_df, fields={'Text': self.TEXT, 'Label': self.LABEL})
    ...
    def _train_model(net, dataloaders_dict, criterion, optimizer, num_epochs, patience):
        ...
        for epoch in range(num_epochs):
            ...
            for phase in ['train', 'val']:
                ...
                epoch_f1_score = f1_score(np.concatenate(np.array(ground_truths)), np.concatenate(
                    np.array(predictions)), average=calc_f1_average)
    ...
    def predict(self, test_df):
        ...
        test_dl = torchtext.data.Iterator(
            test_ds, batch_size=self.batch_size, train=False, sort=False)
